...
// input features (n, c), r, indices (N, r**3), weight (N, r**3) -> output features (N,
// c)
template <typename scalar_t>
__global__ void devoxelize_forward_kernel(int N, int c, int r,
  ...
  if (i < N) {
    const int *indices_ = indices + R * i;
    const scalar_t *weight_ = weight + R * i;
...
// r, input weight (N, r**3), indices (N, r**3), top_grad (N, c) -> bottom grad (n, c)
template <typename scalar_t>
__global__ void devoxelize_backward_kernel(
    int N, int n, int c, int r, const int *__restrict__ indices,
...
at::Tensor devoxelize_backward_cuda(const at::Tensor top_grad,
                                    const at::Tensor indices,
                                    const at::Tensor weight, int n, int r) {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      top_grad.type(), "devoxelize_backward_cuda", ([&] {
        devoxelize_backward_kernel<scalar_t><<<N, c>>>(
            N, n, c, r, indices.data_ptr<int>(), weight.data_ptr<scalar_t>(),
