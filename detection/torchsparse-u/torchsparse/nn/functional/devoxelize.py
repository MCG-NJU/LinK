...
class DevoxelizeFunction(Function):
    ...
    def forward(ctx, feats: torch.Tensor, coords: torch.Tensor,
                weights: torch.Tensor, r: int) -> torch.Tensor:
        ...
        ctx.for_backwards = (coords, weights, feats.shape[0], r)
        return output

    @staticmethod
    @custom_bwd
    def backward(ctx, grad_output: torch.Tensor):
        coords, weights, input_size, r = ctx.for_backwards
        grad_output = grad_output.contiguous()

        if grad_output.device.type == 'cuda':
            grad_feats = torchsparse.backend.devoxelize_backward_cuda(
                grad_output, coords, weights, input_size, r)
...
def spdevoxelize(feats: torch.Tensor, coords: torch.Tensor,
                 weights: torch.Tensor, r=2) -> torch.Tensor:
    return DevoxelizeFunction.apply(feats, coords, weights, r)
