...
def example_to_device(example, device, non_blocking=False) -> dict:
    ...
    for k, v in example.items():
        if k in ["anchors", "anchors_mask", "reg_targets", "reg_weights", "labels", "hm",
            ...
            #if v[0].dtype == torch.float32:
            #    example_torch[k] = [res.half().to(device, non_blocking=non_blocking).float() for res in v]
            #else:
            #    example_torch[k] = [res.to(device, non_blocking=non_blocking) for res in v]
        elif k in [
            "voxels",
            ...
            #   example_torch[k] = v.half().to(device, non_blocking=non_blocking).float()
            #else:
            #   example_torch[k] = v.to(device, non_blocking=non_blocking)
        elif k == "calib":
            calib = {}
...
class Trainer(object):
    ...
    def batch_processor_inline(self, model, data, train_mode, **kwargs):
        ...
        if train_mode:
            ...
            # macs, params = profile(model, (example,))
