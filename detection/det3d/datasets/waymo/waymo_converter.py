...
def convert(idx):
    print(f'entering idx = {idx}')
    global fnames
    fname = fnames[idx]
    print(f'idx = {idx}, fname = {fname}')
    dataset = tf.data.TFRecordDataset(fname, compression_type='')

    last_frame = len(list(dataset)) - 1
    if os.path.exists(os.path.join(LIDAR_PATH, 'seq_{}_frame_{}.pkl'.format(idx, last_frame))):
        print(f'idx = {idx} is completed')
        return
    ...
    for frame_id, data in enumerate(dataset):
        ...
        print(f'idx_frame = {idx}_{frame_id} generated')
...
def main(args):
    ...
    #     convert(i)
    with Pool(32) as p: # change according to your cpu
        r = list(tqdm.tqdm(p.imap_unordered(convert, range(len(fnames))), total=len(fnames)))
