...
class CenterHead(nn.Module):
    def __init__(
        ...
        self.box_n_dim = 9 if 'vel' in common_heads else 7  # seems not used
    ...
    def predict(self, example, preds_dicts, test_cfg, **kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            if double_flip:
                assert batch_size % 4 == 0, batch_size
    ...
    def post_processing(self, batch_box_preds, batch_hm, test_cfg, post_center_range, task_id):
        ...
        for i in range(batch_size):
            ...
            # if tt_rotation applied, rotate pred boxes back
            if test_cfg.get("tt_rotation", 0) != 0:
                boxes_np = selected_boxes.cpu().detach().numpy()
                rev_angle = -test_cfg.tt_rotation
                boxes_np[:, :3] = box_np_ops.rotation_points_single_angle(
                    boxes_np[:, :3], rev_angle, axis=2
                )
                if boxes_np.shape[1] > 7:
                    boxes_np[:, 6:8] = box_np_ops.rotation_points_single_angle(
                        np.hstack([boxes_np[:, 6:8], np.zeros((boxes_np.shape[0], 1))]),
                        rev_angle,
                        axis=2,
                    )[:, :2]
                boxes_np[:, -1] += rev_angle
                selected_boxes = torch.from_numpy(boxes_np)
                # print(f"pred boxes rotate back! {rev_angle}")
