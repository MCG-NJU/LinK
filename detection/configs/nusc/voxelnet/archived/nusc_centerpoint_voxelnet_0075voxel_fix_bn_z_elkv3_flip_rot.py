...
model = dict(
    ...
    backbone=dict(
        type="SpMiddleResNetFHDELKv3", num_input_features=5, ds_factor=8
    ),
    neck=dict(
        type="RPN",
        layer_nums=[5, 5],
...
test_pipeline = [
    ...
    dict(type="TTRotation") if TT_ROTATION else dict(type="Empty"), 
    dict(type="Voxelization", cfg=voxel_generator),
    dict(type="AssignLabel", cfg=train_cfg["assigner"]),
    dict(type="Reformat", double_flip=DOUBLE_FLIP, tt_rotation=TT_ROTATION),
]
